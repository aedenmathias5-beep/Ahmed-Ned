      ...
      <div className="container-max">
        <div className="max-w-4xl mx-auto">
          <div className={`relative rounded-3xl overflow-hidden transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="absolute inset-0 bg-gradient-to-br from-cream via-cream to-surface dark:from-gray-900 dark:via-gray-900 dark:to-gray-800" />
            <div className="absolute top-0 left-0 w-1.5 h-full bg-gradient-to-b from-coral via-gold to-teal" />
            <div className="absolute top-0 right-0 w-40 h-40 bg-coral/5 rounded-full blur-3xl -translate-y-10 translate-x-10" />

            <div className="relative p-8 md:p-12">
              <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
                <div className="flex-shrink-0 relative">
                  <div className="absolute -inset-3 border border-coral/15 rounded-2xl animate-pulse" style={{ animationDuration: '4s' }} />
                ...
                <div className="text-center md:text-left flex-1">
                  <p className="text-coral text-xs tracking-[0.3em] uppercase font-semibold mb-3">Votre professeur</p>
                  <h2 className="font-display text-3xl md:text-4xl font-bold text-navy dark:text-white mb-2">Monsieur Nedjar</h2>
                  <p className="text-lg text-navy dark:text-coral font-medium mb-3">Professeur Agrégé de Mathématiques</p>
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start gap-3 mt-5">
                    {['🎓 Agrégé', '🏫 En activité', '📋 Examinateur', '💙 Inclusif'].map((tag, i) => (
                      <span
                        key={tag}
                        className={`text-xs bg-white dark:bg-gray-800 px-4 py-2 rounded-full text-navy dark:text-gray-200 font-medium border border-gray-100 dark:border-gray-700 shadow-sm card-premium transition-all duration-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
